...
function groupPath(address: string, homeDir?: string): string {
  return join(groupsDir(homeDir), `${address}.json`);
}
...
export function loadGroup(addressOrName: string, homeDir?: string): GroupFile | null {
  ...
  return (
    listGroups(homeDir).find(
      (g) =>
        g.name === addressOrName || g.groupAddress.toLowerCase() === addressOrName.toLowerCase(),
    ) ?? null
  );
}
