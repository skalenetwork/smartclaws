...
describe("config", () => {
  ...
  test("createDefaultConfig sets v2 identity fields correctly", () => {
    const config = createDefaultConfig("base-testnet", "https://rpc.example.com", 1351057110, "0xABC", "bridge-agent", "0xwallet");
  ...
  test("assertHomeWallet rejects a mismatched wallet", () => {
    const config = createDefaultConfig("local", "http://127.0.0.1:8545", 31337, "0xregistry", "controller", "0xabc");
