...
import { enforceModeConstraints, resolveAgent, resolveDevice, resolveGroup } from "../../src/services/discovery.js";
...
describe("discovery resolution", () => {
  ...
  test("resolves a device by local cache name inside a group", async () => {
    ...
    saveDevice(device("0x0000000000000000000000000000000000000031", "sensor", groupAddress), tempDir);
  ...
  test("fails ambiguous local device names", async () => {
    ...
    writeFileSync(
      join(dir, "first.json"),
      JSON.stringify(device("0x0000000000000000000000000000000000000031", "sensor", "0x0000000000000000000000000000000000000011")),
    );
    writeFileSync(
      join(dir, "second.json"),
      JSON.stringify(device("0x0000000000000000000000000000000000000032", "sensor", "0x0000000000000000000000000000000000000012")),
    );
...
describe("mode constraints", () => {
  ...
  const deviceRecord = device("0x0000000000000000000000000000000000000031", "sensor", groupRecord.groupAddress);
  ...
  test("bridge-agent requires one agent and exactly one device", () => {
    expect(() => enforceModeConstraints("bridge-agent", { agent: agentRecord, devices: [] })).toThrow();
    expect(() => enforceModeConstraints("bridge-agent", { agent: agentRecord, devices: [deviceRecord] })).not.toThrow();
  });

  test("master-agent requires one agent, one group, and devices from that group", () => {
    expect(() => enforceModeConstraints("master-agent", { agent: agentRecord, devices: [deviceRecord] })).toThrow();
    expect(() =>
      enforceModeConstraints("master-agent", {
        agent: agentRecord,
        group: groupRecord,
        devices: [device("0x0000000000000000000000000000000000000032", "other", "0x0000000000000000000000000000000000000099")],
      }),
    ).toThrow();
    expect(() =>
      enforceModeConstraints("master-agent", { agent: agentRecord, group: groupRecord, devices: [deviceRecord] }),
    ).not.toThrow();
  });
});
