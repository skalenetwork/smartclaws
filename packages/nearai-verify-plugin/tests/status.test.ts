...
describe("RecordStore session scoping", () => {
  test("returns records only for the caller's session", () => {
    ...
    expect(store.query({ sessionId: "s1", isOwner: false, selector: "latest" })).toHaveLength(1);
  ...
  test("clear drops all records", () => {
    ...
    store.clear();
    expect(store.query({ sessionId: "s1", isOwner: false, selector: "latest" })).toHaveLength(0);
  });
