...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-xs font-medium text-muted-foreground/60 pt-4 pb-1">
      {children}
    </p>
  );
}
        ...
        <div className="ml-4 pl-3 border-l border-border/50 space-y-0.5 py-1">
          {isLoading ? (
            ...
            devices.map((device: DeviceInfo) => {
              const isDeviceActive = location.pathname === `/devices/${device.address}`;
              ...
              return (
                ...
                >
                  <AddressAvatar address={device.address} size={18} kind="device" />
                  <span className="truncate flex-1">
                    {device.devName || `${device.address.slice(0, 8)}…${device.address.slice(-4)}`}
                  </span>
                  <span title={label} className={cn("h-1.5 w-1.5 rounded-full shrink-0 mr-1.5", dotColors[color])} />
                </Link>
              );
            })
          )}
