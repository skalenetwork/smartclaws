...
function downsample(
  ...
  for (let i = 0; i < real.length; i += step) {
    ...
    result.push({ ts: chunk[Math.floor(chunk.length / 2)].ts, value: Math.round(avg * 100) / 100 });
  }
  return result;
}
  ...
  return (
    <div className="bg-card rounded-xl border p-4 shadow-sm min-w-0">
      <div className="flex items-center justify-between mb-3 gap-2">
        <h3 className="text-sm font-medium text-muted-foreground capitalize shrink-0">{title}</h3>
        <div className="flex items-center gap-3 min-w-0">
          {stats && (
            <div className="hidden sm:flex gap-3 text-[10px] text-muted-foreground/70 shrink-0">
              <span>min: <span className="text-foreground/70">{stats.min.toFixed(2)}</span></span>
              <span>avg: <span className="text-foreground/70">{stats.avg.toFixed(2)}</span></span>
              <span>max: <span className="text-foreground/70">{stats.max.toFixed(2)}</span></span>
            </div>
          )}
