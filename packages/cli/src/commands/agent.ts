...
agentCommand
  ...
  .action(async (opts) => {
    ...
    try {
      const result = await grantAgentPermission(config, wallet, opts.agent, role, opts.account);
...
agentCommand
  ...
  .action(async (opts) => {
    ...
    try {
      const result = await revokeAgentPermission(config, wallet, opts.agent, role, opts.account);
