...
deviceCommand
  ...
  .action(async (opts) => {
    ...
    try {
      const result = await grantDevicePermission(config, wallet, opts.device, role, opts.account);
...
deviceCommand
  ...
  .action(async (opts) => {
    ...
    try {
      const result = await revokeDevicePermission(config, wallet, opts.device, role, opts.account);
...
deviceCommand
  ...
  .action(() => {
    ...
    for (const d of devices) {
      ...
      if (d.createdAt) console.log(`  Created:   ${new Date(d.createdAt * 1000).toISOString()}`);
    }
  });
