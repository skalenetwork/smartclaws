...
describe("SmartClawsChannel", function () {
  ...
  describe("Publishing", function () {
    ...
    it("should reject a payload exceeding capacity", async function () {
      ...
      await expect(
        tiny.publishMessage(tooBig)
      ).to.be.revertedWithCustomError(tiny, "PayloadExceedsCapacity");
    });
  });
  ...
  describe("Publisher management", function () {
    ...
    it("should reject adding the owner as publisher", async function () {
      await expect(
        channel.addPublisher(owner.address)
      ).to.be.revertedWithCustomError(channel, "CannotModifyOwnerAsPublisher");
    });

    it("should reject adding a duplicate publisher", async function () {
      await channel.addPublisher(publisher.address);
      await expect(
        channel.addPublisher(publisher.address)
      ).to.be.revertedWithCustomError(channel, "PublisherAlreadyAuthorized");
    });
    ...
    it("should reject removing a non-publisher", async function () {
      await expect(
        channel.removePublisher(publisher.address)
      ).to.be.revertedWithCustomError(channel, "PublisherNotAuthorized");
    });

    it("should reject removing the owner as publisher", async function () {
      await expect(
        channel.removePublisher(owner.address)
      ).to.be.revertedWithCustomError(channel, "CannotModifyOwnerAsPublisher");
    });
  ...
  describe("Writes disabled", function () {
    ...
    it("should reject disableWrites from a non-owner, non-registry caller", async function () {
      await expect(
        channel.connect(other).disableWrites()
      ).to.be.revertedWithCustomError(channel, "Unauthorized");
    });
  ...
  describe("Pausing", function () {
    ...
    it("should reject duplicate pause and unpause when already unpaused", async function () {
      await expect(channel.unpause()).to.be.revertedWithCustomError(
        channel,
        "ExpectedPause"
      );
      await channel.pause();
      await expect(channel.pause()).to.be.revertedWithCustomError(
        channel,
        "EnforcedPause"
      );
    });
