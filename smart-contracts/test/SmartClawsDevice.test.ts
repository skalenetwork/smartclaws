...
import type { SmartClawsDevice, SmartClawsChannel } from "../types/ethers-contracts/index.js";
...
describe("SmartClawsDevice", function () {
    ...
    describe("pause", function () {
        ...
        it("should reject pause from a non-DEVICE_ADMIN", async function () {
            await expect(device.connect(publisher).pause()).to.be.revertedWithCustomError(
                device,
        ...
        it("should reject unpause from a non-DEVICE_ADMIN", async function () {
            ...
            await expect(device.connect(publisher).unpause()).to.be.revertedWithCustomError(
                device,
                "AccessControlUnauthorizedAccount",
            );
        });
    ...
    describe("prune", function () {
        ...
        it("should reject prune from a non-DEVICE_ADMIN", async function () {
            await expect(device.connect(publisher).pruneOutgoing(1)).to.be.revertedWithCustomError(
                device,
                "AccessControlUnauthorizedAccount",
            );
        });
    ...
    describe("deactivate", function () {
        it("should reject deactivate from a non-DEFAULT_ADMIN", async function () {
            await expect(device.connect(deviceAdmin).deactivate()).to.be.revertedWithCustomError(
                device,
                "AccessControlUnauthorizedAccount",
            );
        });
