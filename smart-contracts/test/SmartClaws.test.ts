...
describe("SmartClaws", function () {
  ...
  describe("Deployment", function () {
    ...
    slots.forEach((label, index) => {
      it(`should reject a zero ${label} factory`, async function () {
        ...
        await expect(
          SmartClawsFactory.deploy(...args)
        ).to.be.revertedWithCustomError(registry, "InvalidFactoryAddress");
      });
    });
  });
  ...
  describe("Channels", function () {
    ...
    it("should reject deleting an unregistered channel", async function () {
      await expect(
        registry.deleteChannel(other.address)
      ).to.be.revertedWithCustomError(registry, "ChannelNotRegistered");
    });
  ...
  describe("Device groups", function () {
    ...
    it("should store the device group creation timestamp", async function () {
      ...
      const groupAddr = getAddressFromReceipt(registry, receipt, "DeviceGroupRegistered", "deviceGroup");
  ...
  describe("Agents", function () {
    ...
    it("should store the agent creation timestamp", async function () {
      const tx = await registry.connect(owner).registerAgent("timed-agent", "metadata", ONE_MB);
      const receipt = await tx.wait();
    ...
    it("should reject unregistering an unknown agent", async function () {
      await expect(
        registry.unregisterAgent(other.address)
